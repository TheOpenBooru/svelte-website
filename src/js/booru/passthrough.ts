...
export async function post_edit(post_id: number, source: string, rating: Types.Rating, tags: string[]): Promise<void> {
    return await Post.edit(post_id, source, rating, tags, getBooruConfig())
}

export async function post_get(post_id: number): Promise<Types.Post> {
    return await Post.get(post_id, getBooruConfig())
}

export async function post_delete(post_id: number): Promise<void> {
    return await Post.Delete(post_id, getBooruConfig())
}

export async function add_upvote(post_id: number): Promise<void> {
    return await Post.add_upvote(post_id, getBooruConfig())
}

export async function remove_upvote(post_id: number): Promise<void> {
    return await Post.remove_upvote(post_id, getBooruConfig())
}

export async function add_downvote(post_id: number): Promise<void> {
    return await Post.add_downvote(post_id, getBooruConfig())
}

export async function remove_downvote(post_id: number): Promise<void> {
